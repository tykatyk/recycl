...
const queryValidationSchema = yup.object({
  ...
  variant: yup
    .string<Variant>()
    .transform((value) => (documentActivityStatus[value] ? value : undefined))
...
async function collectionPointsHanlder(
  ...
  switch (req.method) {
    ...
    case 'GET':
      ...
      await dbConnect()
      const collectionPoints = await eventQueries.getAll(validatedQuery, userId)

      const total = await CollectionPointModel.countDocuments({
        status: validatedQuery.variant,
        user: userId,
      })
